...
void Change_Score_course_1(Node_cou* pCur_cou, wchar_t* file_login, int &n) {
	if (pCur_cou->cou.count != 0) {
		std::wstring str_1;
		std::wstring str_2;
		std::wifstream fin(L"Default_time.txt");
		...
		system("cls");
		Change_Score_course_2(pCur_cou, pCur_stu, file_login,n);
		delete[] current_year;
		delete[] current_semester;
		Delete_Node_stu(pHead_stu);
	}
...
void Change_Score_course_2(Node_cou* pCur_cou, Node_stu* pCur_stu, wchar_t* file_login, int &n) {
	system("cls");
	std::wstring str_1;
	std::wstring str_2;
	std::wifstream fin(L"Default_time.txt");
	...
	do {
		...
		if (key == 27)
		{
			Change_Score_course_1(pCur_cou, file_login, n);
		}
	} while (key != 27);
}
		...
		while (1) {
			if (_kbhit()) {
				...
				if (key == 13) {
					switch (pointer) {
					case 0:
						Input_score(pCur_cou, file_login, n);
						Sleep(100000);
						break;
					case 1:
